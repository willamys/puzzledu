...
isc.ListGrid.addProperties( {
    ...
    _formatLinkCellValue : function (value, field, grid, record, rowNum, colNum) {
        ...
        var linkText = (record && record[grid.linkTextProperty]) ? record[grid.linkTextProperty] 
                                                            : field[grid.linkTextProperty] || value;
    ...
    _linkClicked : function (event, rowNum, colNum) {
        ...
        if (event.target == "javascript:void") {
            mustCancel=true;
    ...
    // enter-keypress or filter button click. +link{listGrid.autoFetchAsFilter} determines whether
    // the filter occurs as a +link{listGrid.fetchData()} or +link{listGrid.filterData()} call.
    // <P>
    ...
    //> @attr listGrid.autoFetchAsFilter (boolean : true : IR)
    // Determines whether +link{filterData()} or +link{fetchData()} should be called when this grid
    // is initially filtered via +link{autoFetchData}, or filtered by the user via the
    // +link{showFilterEditor,filterEditor}. 
    // @group dataBinding
    // @visibility external
    //<
    // implemented in recordEditor.performAction()
    ...
    // edit mode. When this attribute is set, if clicking on the field will change the value - for
...
//   Note that if +link{listGrid.selectionAppearance} is <code>"checkbox"</code>, only selectionType
//   "simple" is supported. 
// @visibility external
//<
setSelectionType : function (selectionType, ignoreCheckbox) {
    // If selectionAppearance is checkbox only SIMPLE selectionType is supported 
    // in this case just store the specified selectionType so if we change selectionAppearance
    // in the future we respect it
    // Exception - the addional ignoreCheckbox param bypasses this and allows setSelectionAppearance
    // to actually set the selectionType to "simple" after setting the appearance to checkbox and
    // storing out the old selectionType value.
    if (!ignoreCheckbox && this.selectionAppearance == "checkbox") {
        this._explicitSelectionType = selectionType;
        return;
    }
...
setSelectionAppearance : function (selectionAppearance) {
    if (this.selectionAppearance == selectionAppearance) return;
    
    this.selectionAppearance = selectionAppearance;
    if (selectionAppearance == "checkbox") {
        // store out the old selection type so we can revert to it when the selectionAppearance is
        // changed
        this._explicitSelectionType = this.selectionType;
        this.setSelectionType(isc.Selection.SIMPLE, true);
    } else {
        this.setSelectionType(this._explicitSelectionType || isc.Selection.MULTIPLE);
    }
...
_observeData : function (data) {
    ...
    if (isc.ResultSet && isc.isA.ResultSet(data)) {
        this.observe(data, "dataArrived", "observer.dataArrived(arguments[0],arguments[1])");
    } else if (isc.ResultTree && isc.isA.ResultTree(data)) {
        this.observe(data, "dataArrived", "observer.dataArrived(arguments[0])");
    }
...
getFieldState : function (includeTitle) {
    ...
    if (this.completeFields) {
        ...
        if (this.frozenFields) {
            var frozenMembers = this.frozenHeader.members;
        }
        for (var i = 0; i < this.completeFields.length; i++) {
            var field = this.completeFields[i],
            ...
            if (includeTitle) fieldState.title = field.title;
            if (members) {
...
getCellValue : function (record, recordNum, fieldNum, gridBody) {
    ...
    if (this.isCheckboxField(field) && !record._isGroup) {
        ...
        var icon = isSel ? (this.checkboxFieldTrueImage || this.booleanTrueImage)
...
getInitialCriteria : function () {
    ...
    if (!hasInitialFieldValue) {
        ...
        isc.addProperties(initialCriteria, this.initialCriteria);
    }
...
// empty value.  By default returns +link{Validators.requiredField}.
...
// Remove a record from this listGrids +link{listGrid.data,data object}.
// <P>
// If this grid is bound to a dataSource, and +link{listGrid.saveLocally} is not true,
// this method will perform a DataSource "remove" operation to remove records from this component's
// DataSource.
// <P>
// Otherwise the data will be removed from the grids data array. 
//
// @param data (Object) listGrid record, or primary key values of record to delete.
...
removeData : function (recordKeys, callback, requestProperties) {
    ...
    if (this.data) this.data.remove(recordKeys);
...
drop : function () {
    ...
    var dropIndex = this.getDropIndex(null, null);
...
// <P>
// If parameter <i>recordNum</i>
// is not passed, the current event row is used, see +link{listGrid.getEventRow()}. 
// <P>
...
getDropIndex : function (recordNum, reorderPosition) {
    ...
    if (this.canReorderRecords) {
    	...
    	if (recordNum == -2) {
            recordNum = this.data.getLength() - 1;
            reorderPosition = this.getReorderPosition(recordNum);
        }
    }
...
_setCheckboxHeaderState : function (state) {
    ...
    var icon = state ? (this.checkboxFieldTrueImage || this.booleanTrueImage)
                     : (this.checkboxFieldFalseImage || this.booleanFalseImage),
         title = this.getValueIconHTML(icon, field);
...
headerClick : function (headerFieldNum, header) {
    ...
    if (this.isCheckboxField(field)) {
        if (field._allSelected) {
            ...
            // If we're showing an incomplete resultSet we can't select all records.
            // the above call will already show a warning dialog to the user.
            // Avoid showing the checked image in the header in this case
            if (!isc.ResultSet || !isc.isA.ResultSet(this.data) ||
                this.data.allMatchingRecordsCached()) {
...
regroup : function (fromSetData) { 
	...
	if (!this.isGrouped || (this.canGroupBy == false) || !this.groupByField || 
        ...
        !(this.originalData || this.data)) return;
	...
	this.groupTree = this.createAutoChild("groupTree", {
        ...
        // Otherwise, for a DataSource that declares a tree 
        // relationship, ResultTree will pick these attributes based on the DataSource and behave
        // badly, clobbering the parentId field with parent pointers related to grouping, and
        // checking for and clearing duplicates on the idField
...
groupBy : function (groupFieldName) {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        // just in case
        if (this.isCheckboxField(fields[i])) continue;
        // if the field has a displayField and no optionDataSource, use the displayField
        // instead of the actual field passed in
        var fld = this.getSpecifiedField(fields[i]);
        if (fld && fld.displayField != null && this.getField(fld.displayField) 
            && fld.optionDataSource == null) {
            this.groupByField.add(fld.displayField);    
        } else {
...
isc.ListGrid.registerStringMethods({
    ...
    // @param startRow (integer) starting index of the newly loaded set of rows
    // @param endRow (integer) ending index of the newly loaded set of rows (non inclusive).
    // @visibility external
    //<
