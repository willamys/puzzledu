...
drop : function () {
    ...
    // check if originating tile is in a position lower than the destination. Augment the dest
    // index if it is, otherwise transferRecords puts it in a slot too early because array.slideList()
    // doesn't count objects present both in the source and dest arrays when building the
    // new array. 
    // TODO may want to make slideList() handle self-rearranges
    var fromIndex = this.tiles.findIndex("ID", dropRecords[0].ID);
    if (fromIndex != -1 && fromIndex < index) index += dropRecords.length; 
